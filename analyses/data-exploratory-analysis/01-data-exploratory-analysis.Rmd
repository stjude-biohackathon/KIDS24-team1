...
For demo purposes, we use dummy data cohort and subset by human brain tumor samples. In addition, we generate random fake SJUID per brain cancer type as this information is not contained in the demo cohort. Finally, we investigate the number of samples per `cancer_type_brain` and `Assay`.
