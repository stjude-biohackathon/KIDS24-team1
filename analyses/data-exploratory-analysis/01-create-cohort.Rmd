...
longitudinal_df <- cbind(df, longitudinal) %>% 
  #left_join(longitudinal_bind_df, by = "SJUID", relationship = "many-to-many") %>% 
  unique() %>% 
...
# Save file
write.csv(longitudinal_df, file.path(input_dir, "cohort-longitudinal.csv"), row.names = FALSE)
```
