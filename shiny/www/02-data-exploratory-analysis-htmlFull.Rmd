...
attach(params)
if (dir.exists(file.path("CohortOut", user))) {
  unlink(file.path("CohortOut", user), recursive=TRUE)
  dir.create(file.path("CohortOut", user))
} else {
  dir.create(file.path("CohortOut", user))
}
analysis_dir <- file.path("CohortOut", user)
