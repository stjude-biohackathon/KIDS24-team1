...
<br />
**Author: <a href="mailto:achroni@stjude.org">Antonia Chroni</a>**
<br />
...
This is an exploratory analysis of the data availability in terms of assays in the Comprehensive Omics Catalog for Hartwell. This is critical for mitigating duplicate sequencing requests and efforts through Hartwell. This notebook aims to showcase:
...
attach(params)
if (dir.exists(file.path("FragLongOut", user))) {
  unlink(file.path("FragLongOut", user), recursive=TRUE)
  dir.create(file.path("FragLongOut", user))
} else {
  dir.create(file.path("FragLongOut", user))
}
analysis_dir <- file.path("FragLongOut", user)
input_dir <- file.path("input") 

# We will first read in metadata file as we need to define sample_name 
metadata_file <- file.path(input_file) # metadata input file
palette_file <- file.path("palettes", "assay_color_palette.tsv")
tumor_palette_file <- file.path("palettes", "tumor_color_palette.tsv")
longitudinal_palette_file <- file.path("palettes", "longitudinal_color_palette.tsv")
...
source(paste0("scripts/function-create-barplot.R"))
source(paste0("scripts/theme_plot.R"))
```
